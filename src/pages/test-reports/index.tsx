...
import { getTestReports, deleteTestReportById } from 'apiSdk/test-reports';
import { TestReportInterface } from 'interfaces/test-report';

type ColumnType = ColumnDef<TestReportInterface, unknown>;
...
export function TestReportListPage(props: TestReportListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getTestReports({
        relations: ['patient', 'user', 'hospital'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TestReportInterface>>(
    () => `/test-reports?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TestReportInterface) => {
    if (hasAccess('test_report', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/test-reports/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/test-reports/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('test_report', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/test-reports/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('test_report', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="test_report" />
        </Flex>

        {hasAccess('test_report', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/test-reports/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
