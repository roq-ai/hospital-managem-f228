...
import { getTestReportById } from 'apiSdk/test-reports';
import { TestReportInterface } from 'interfaces/test-report';

function TestReportViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<TestReportInterface>(
    () => (id ? `/test-reports/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('test_report', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/test-reports/edit/${id}`} passHref legacyBehavior>
                    <Button
